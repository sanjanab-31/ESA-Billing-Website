...
const ClearAndReseed = () => {
    ...
    const clearAllData = async () => {
        if (!user?.uid) {
            setMessage('❌ You must be signed in to clear data.');
            return;
        }
        setStep('clearing');
        setMessage('Clearing your billing data...');

        try {
            const uid = user.uid;
            const collections = ['customers', 'products', 'invoices', 'payments'];
            for (const colName of collections) {
                const snap = await getDocs(collection(db, 'users', uid, colName));
                for (const d of snap.docs) {
                    await deleteDoc(doc(db, 'users', uid, colName, d.id));
                }
            }
            // Clear settings doc
            ...
            setStep('cleared');
            setMessage('✅ Your billing data has been cleared.');
        } catch (error) {
