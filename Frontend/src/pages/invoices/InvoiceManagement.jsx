                ...
                <tbody className="divide-y">
                  {previewData.items.map((item, index) => (
                    <tr key={item.id}>
                      ...
                      <td className="px-4 py-3">₹{item.rate.toLocaleString()}</td>
            ...
            {filteredInvoices.length > 0 ? (
              filteredInvoices.map((invoice) => (
                <div key={invoice.id} className="px-6 py-4 hover:bg-gray-50 transition-colors">
                  <div className="grid grid-cols-7 gap-6 items-center text-sm">
...
const InvoiceManagementSystem = () => {
  ...
  const sampleInvoices = [
    {
      ...
      client: { id: 1, name: 'TechnoFab Industries', address: '123 Industrial Area, Mumbai', gst: '27ABCDE1234F1Z5' },
      amount: 147500, // Corrected total
      dueDate: '2024-02-14',
      status: 'Paid',
      items: [
    ...
    },
    {
      ...
      client: { id: 2, name: 'Digital Solutions Ltd', address: '456 Tech Park, Bangalore', gst: '29FGHIJ5678K2L6' },
      amount: 100300, // Corrected total
      dueDate: '2024-02-15',
      status: 'Draft',
      items: [
    ...
    },
    {
      ...
      client: { id: 3, name: 'Tech Innovators Pvt Ltd', address: '789 Innovation Hub, Pune', gst: '27MNOPQ9012R3S7' },
      amount: 295000, // Corrected total
      dueDate: '2024-02-16',
      status: 'Overdue',
      items: [
  ...
  const saveDraft = () => {
    const draftInvoice = {
      ...invoiceData,
      id: editingInvoice ? editingInvoice.id : Date.now(), // Preserve ID if editing
      status: 'Draft',
      amount: calculations.total
    };
    
    if (editingInvoice) {
        setInvoices(prev => prev.map(inv => inv.id === editingInvoice.id ? draftInvoice : inv));
    } else {
...
${invoice.items.map(item =>
  `${item.description.padEnd(30)} | HSN: ${item.hsnCode.padEnd(8)} | Qty: ${String(item.quantity).padEnd(5)} | Rate: ₹${item.rate.toLocaleString().padEnd(10)} | Amount: ₹${item.amount.toLocaleString()}`
).join('\n')}
-----------------------------------
Subtotal:         ₹${subtotal.toLocaleString()}
CGST (${invoice.cgst}%):        ₹${cgstAmount.toLocaleString()}
SGST (${invoice.sgst}%):        ₹${sgstAmount.toLocaleString()}
...
Status: ${invoice.status}
  ...
  return (
    <div>
      ...
      {showPreview && (
        <InvoicePreview 
          // Pass the correct data for preview depending on the context
          invoice={selectedInvoice} // For viewing an existing invoice
          invoiceData={currentPage !== 'management' ? invoiceData : null} // For previewing a new/edited invoice
          calculations={calculations}
          setShowPreview={setShowPreview} 
        />
      )}
